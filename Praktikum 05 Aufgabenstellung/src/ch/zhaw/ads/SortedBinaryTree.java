...
public class SortedBinaryTree<T extends Comparable<T>> implements Tree<T> {
    ...
    private void nextNode(TreeNode<T> node, int previousHeight) {
        node.height = previousHeight + 1;
        height = Math.max(node.height, height);
        if (node.left != null) {
            nextNode(node.left, node.height);
        }

        if (node.right != null) {
            nextNode(node.right, node.height);
        }
    }
...
//        Traversal<T> traversal = new TreeTraversal<>(p);
//        traversal.levelorder(visitor -> size++);
//        return size;
        if (p.left != null) calcSize(p.left);
        size++;
        if (p.right != null) calcSize(p.right);
        return size;
    }
